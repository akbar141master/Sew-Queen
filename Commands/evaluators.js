...
SewQueen['IntroduceCMD']({pattern: 'pms ?(.*)', fromMe: true, desc: pmmm, onlyGroup: true }, (async (message, input) => {
...
SewQueen['IntroduceCMD']({pattern: 'pmtts ?(.*)', fromMe: true, desc: psmm, onlyGroup: true}, (async (message, input) => {
    if (!message.reply_message) return await message.client.sendMessage(message.jid,NDATA.NEED_REPLY, MessageType.text);
    if (message.reply_message && input[1] == '') return await message.client.sendMessage(message.jid, NDATA.NEED_WORDS, MessageType.text);
    let 
    ...
    if(langMatch = input[1].input("\\{([a-z]{2})\\}")) {
        LANG = langMatch[1]
        ttsMessage = ttsMessage.replace(langMatch[0], "")
    } 
    if(speedMatch = input[1].input("\\{([0].[0-9]+)\\}")) {
